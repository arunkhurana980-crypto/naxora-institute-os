...
const marker = "// ================= PART 116 — SUBSCRIPTION FEATURE ACCESS CONTROL =================";
...
const precheck = check(serverPath);
...
if (!(finalSource.indexOf(marker) >= 0 && finalSource.indexOf(marker) < find404(finalSource))) {
  fs.writeFileSync(serverPath, original, "utf8");
  fail("Part 116 route-order verification failed. Original server.js restored.");
}

console.log("\nPART 116 APPLIED SUCCESSFULLY");
console.log("Part 116 registered before 404 handler: PASS");
console.log("server.js syntax: PASS");
console.log("Next: node .\\VERIFY-PART116.js\n");
