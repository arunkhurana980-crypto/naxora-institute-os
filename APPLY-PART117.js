...
if (postcheck.status !== 0) {
  ...
  fail(`Part 117 syntax check failed. Original server.js restored.\n${postcheck.stderr || postcheck.stdout}`);
}
...
if (!(finalSource.indexOf(marker) >= 0 && finalSource.indexOf(marker) < find404(finalSource))) {
  fs.writeFileSync(serverPath, original, "utf8");
  fail("Part 117 route-order verification failed. Original server.js restored.");
}

console.log("\nPART 117 APPLIED SUCCESSFULLY");
console.log("Part 117 registered before 404 handler: PASS");
console.log("server.js syntax: PASS");
console.log("Next: node .\\VERIFY-PART117.js\n");
